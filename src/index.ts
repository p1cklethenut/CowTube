...
if (!VERSION || !ACCESS_TOKEN || !PROCESS_PORT) {
  throw new Error("VERSION or ACCESS_TOKEN or PROCESS_PORT not defined from config file.");
}
...
function admin(req: any, res: any): void {
  ...
  if (req.body.pass == ACCESS_TOKEN) {
    const userob = DATAOBJ.users[req.body.value];
    switch (req.body.cmd) {
      case "chat":
        if (req.body.value) {
          io.emit("chatmsg", {
      ...
      case "TIMEBLOCK":
        ...
        res.send(
          `TIMEBLOCK set to till ${TIMEBLOCKingfromnow} or ${new Date(TIMEBLOCK).toISOString()}`,
        );
      ...
      case "eval":
        if (req.body.value) {
          try {
            res.send(String(eval(req.body.value)));
          } catch (e) {
            res.send(e);
          }
        }
        ...
        if (userobcon && userobreq) {
          ...
          if (onlinetoduelclientuser && onlinetoduelrequser) {
            if (onlinetoduelrequser.online && onlinetoduelclientuser.online) {
              ...
              setTimeout(() => {
                if (userobreq && userobcon)
                  if (userobreq.cows || 0 > userobcon.cows || 0) {
                    reward = Math.floor(data.stakes * userobcon.cows);
                  } else {
                    reward = Math.floor(data.stakes * userobreq.cows);
                  }
                if (
                ...
                if (duels[thisduelid]?.player1 && duels[thisduelid]?.player2) {
                  if (
                    duels[thisduelid]?.player1?.clicks ||
                    0 > duels[thisduelid]?.player2?.clicks ||
                    0
                  ) {
